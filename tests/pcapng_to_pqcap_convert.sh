...
if [[ -z "${PQCAP_BIN}" && -x "${ROOT}/../dist/pqcap" ]]; then
  PQCAP_BIN="${ROOT}/../dist/pqcap"
fi

if [[ -n "${PQCAP_BIN}" && -x "${PQCAP_BIN}" ]]; then
  CLI_HTTP="${OUT_DIR}/http_cli.pqcapng"
  "${PQCAP_BIN}" convert "${FIXTURES}/http.pcapng" "${CLI_HTTP}" >/dev/null

  python3 "${ROOT}/tests/lib/pqcap_file_checks.py" assert-indexed "${CLI_HTTP}"
  tshark_must_read "${CLI_HTTP}"

  PLAIN_ETH="$(tshark_count "${FIXTURES}/http.pcapng" "eth")"
  CLI_ETH="$(tshark_count "${CLI_HTTP}" "eth")"
  [[ "${CLI_ETH}" == "${PLAIN_ETH}" ]] || fail "pqcap convert changed eth frame count (${PLAIN_ETH} -> ${CLI_ETH})"
  (( PLAIN_ETH > 0 )) || fail "expected eth frames in http.pcapng"

  CLI_ROWS="$("${PQCAP_BIN}" query -c "SELECT COUNT(*)::BIGINT AS n FROM read_pqcap('${CLI_HTTP}');" | grep -E '^[0-9]+$' | tail -n 1)"
  [[ "${CLI_ROWS}" == "43" ]] || fail "pqcap convert expected 43 metadata rows, got '${CLI_ROWS}'"

  CLI_PACKETS="$("${PQCAP_BIN}" query -c "SELECT COUNT(*)::BIGINT AS n FROM read_pqcap_packets('${CLI_HTTP}');" | grep -E '^[0-9]+$' | tail -n 1)"
  [[ "${CLI_PACKETS}" == "43" ]] || fail "pqcap convert expected 43 packets, got '${CLI_PACKETS}'"

  echo "PASS pqcap convert http.pcapng -> pqcapng (eth/tshark + read_pqcap/read_pqcap_packets)"
else
  echo "SKIP pqcap CLI convert (set PQCAP_BIN to dist/pqcap)"
fi
